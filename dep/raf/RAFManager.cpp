...
std::string RAFManager::findGameBasePath()
{
    //printf("Searching for LoL base path...");
        ...
        for (size_t i = 0; i < strKeyPathCU.size(); i++)
        {
            ...
            {
                //printf("Cannot open key HKEY_CLASSES_ROOT '%s'", strKeyPathCU[i].c_str());
                continue;
            }

            if (RegQueryValueExA(hKey, strKeyName.c_str(), NULL, &dwValueType, (LPBYTE)byteValue, &dwValueSize) != ERROR_SUCCESS)
            {
                //printf("Cannot read key HKEY_CLASSES_ROOT '%s'", strKeyPathCU[i].c_str());
                continue;
            }

            string sValue(byteValue);
            sValue += "/projects/lol_game_client/";
            printf("Found base path in %s", sValue.c_str());
        ...
        for (size_t i = 0; i < strKeyPathCU.size(); i++)
        {
            ...
            {
                //printf("Cannot open key HKEY_CURRENT_USER '%s'", strKeyPathCU[i].c_str());
                continue;
            }

            if (RegQueryValueExA(hKey, strKeyName.c_str(), NULL, &dwValueType, (LPBYTE)byteValue, &dwValueSize) != ERROR_SUCCESS)
            {
                //printf("Cannot read key HKEY_CURRENT_USER '%s'", strKeyPathCU[i].c_str());
                continue;
            }
        ...
        for (size_t i=0; i< strKeyPathLM.size();i++)
        {
            ...
            {
                //printf("Cannot open key HKEY_LOCAL_MACHINE '%s'", strKeyPathCU[i].c_str());
                continue;
            }

            if (RegQueryValueExA(hKey, strKeyName.c_str(), NULL, &dwValueType, (LPBYTE)byteValue, &dwValueSize) != ERROR_SUCCESS)
            {
                //printf("Cannot read key HKEY_LOCAL_MACHINE '%s'", strKeyPathCU[i].c_str());
                continue;
            }
